...
Consciousness = collections.namedtuple('Consciousness', 'time task')
...
def taxi_process(ident, trips, start_time=0):
    ...
    :return: 
    '''
    cx = yield Event(start_time, ident, 'leave garage') # 产出的第一个Event
    for i in range(trips):  # 每次行程都会执行一遍这个代码块
        # 产出一个Event实例，表示拉到了乘客 协程在这里暂停 等待下一次send() 激活
        cx = yield Event(cx.time, ident, 'pick up passenger')
         # 产出一个Event实例，表示乘客下车 协程在这里暂停 等待下一次send() 激活
        cx = yield Event(cx.time, ident, 'drop off passenger')
    # 指定的行程数量完成后，for 循环结束，最后产出 'going home' 事件。协程最后一次暂停
    yield Event(cx.time, ident, 'going home')
...
class Simulator:
    ...
    def run(self, end_time):
        ...
        while sim_time < end_time:
            ...
            try:
                next_event = active_proc.send(Consciousness(next_time, 'foo'))  # 把计算得到的时间发送给出租车协程。协程会产出下一个事件，或者抛出 StopIteration
